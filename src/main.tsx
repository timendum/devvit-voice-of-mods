...
import { Comment, Post, Devvit } from '@devvit/public-api';
...
const writeCommentForm = Devvit.createForm(
  ...
  async (event, context) => {
    ...
    let body = event.values.body?.trim() || "";
    if (body.length < 1) {
    ...
    if (event.values.commentId !== "-") {
      const parentComment = await context.reddit.getCommentById(event.values.commentId);
...
Devvit.addMenuItem({
  ...
  onPress: async (event, context) => {
    ...
    console.log(`Permissions of ${context.userId} for ${context.subredditName}:`, permissions);
    if (!permissions.includes("mail") && !permissions.includes("all")) {
      context.ui.showToast('You do not have permission to reply as a moderator.');
      return;
    }
